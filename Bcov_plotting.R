...
align_specy_table <- function(path, replace_pattern2='', ref_name, prefix='')  {
  ...
  for(i in seq_along(A)) {
    ...
    class[i,2] <- virus
    contamination <- length(B[,1][grep(B[,1], pattern = '^ref')])
  ...
  colnames(class)[1:4] <- c('Unmap', 'Virus', 'yeast', 'Host')
  return(class)
}



class <- align_specy_table(path = 'Analysis/bovine/host_v2/', prefix = 'qname'
                           , ref_name = 'U00735.2')

write.csv(x = class, file = '~/Analysis/bovine/lin_plot/bovine/align_species.csv')
...
bed <- read_as_list(path='~/Analysis/bovine/viral_v2/') 

A <- fragment_table(list = bed, max_frag = 40 , proportion = F, re_organ = F)
write.csv(A, file = '~/Analysis/bovine/lin_plot/bovine/bovine_viral_fragment.csv')
...
for(j in seq(2,5,1)) {
  bed%>%
    ...
    specific_reads_sgRNA_identify(gap = 50, length = 31032
                                  , gff_path = '~/Analysis/reference/bovine_edit_gff.csv'
                                  , leader = 70, UTR5 = 210, UTR3 = 30741, frag = j) %>%
    specific_frag_recom_table(frag = j, write = T, out_prefix = 'MHV'
                              , write_path = '~/Analysis/bovine/lin_plot/bovine/recombination/')
}
...
frag2 <- read_as_list(path = '~/Analysis/bovine/viral_v2/') %>%
  specific_frag_format_organ(frag = 2) %>% 
  specific_reads_sgRNA_identify(gap = 50, length = 31032
                                , gff_path = '~/Analysis/reference/bovine_edit_gff.csv'
                                , leader = 70, UTR5 = 210, UTR3 = 30741, frag = 2)

lib_matrix <- lib_recom_matrix(lib_list = frag2, frag = 2, libs=8)

frag1 <- read_as_list(path = '~/Analysis/bovine/viral_v2/') %>%
  specific_frag_format_organ(frag = 1)  %>%
  lin_frag1_identification(recom_matrix = lib_matrix, libs = 8
                           , gff_path = '~/Analysis/reference/bovine_edit_gff.csv'
                           , UTR5 = 210, UTR3 = 30741, length = 31032)%>%
  specific_frag_recom_table(frag = 1, write = T, out_prefix = 'bovine'
                            , write_path = '~/Analysis/bovine/lin_plot/bovine/recombination/')
...
bed <- read_as_list(path = '~/Analysis/bovine/viral_v2/')
for(j in seq(2,5,1)) {
  write_path <- paste('~/Analysis/bovine/flanking/', 'frag', j, '/',sep = '')
  ...
  bed%>%
    ...
    get_flanking(frag = j, flanking = 20, 
                 genome_length = 31032, writeto_bed = T
...
for(j in seq(2,5,1)) {
  ...
  frag_flanking <- org_flanking_fasta(path = paste('~/Analysis/bovine/flanking/frag', j, '/', sep='')
                                      , prefix = '.fa')
  
  final_output_generation(list_recom = frag, frag = j
                          , list_flanking_fa = frag_flanking
                          , write = T, write_path = '~/Analysis/bovine/lin_plot/bovine/flanking/')
}
...
write_path <- paste('~/Analysis/bovine/flanking/frag', 1, '/',sep = '')
dir.create(write_path)
...
frag_flanking <- org_flanking_fasta(path = '~/Analysis/bovine/flanking/frag1/', prefix = '.fa')

out_list <- oneF_final_output_generation(list_recom = frag1
                                         , list_flanking_fa = frag_flanking
                                         , write = T
                                         , write_path = '~/Analysis/bovine/lin_plot/bovine/flanking//')
...
for(j in seq(2,5)) {
  ...
  frag_10bp <- bed %>%
    ...
    specific_reads_sgRNA_identify(gap = 50, length = 31032
                                  , gff_path = '~/Analysis/reference/bovine_edit_gff.csv'
                                  , leader = 70, UTR5 = 210, UTR3 = 30741, frag = j) %>%
    cut_by_n_bp(genome_length = 31032, bin = 10, frag=j)
  ...
  #     }
  #     color <- c('black','#93E426','#7706E4')
  #     if(length(table(two$Group))!=3) {
  #       color <- c('#93E426','#7706E4')
  #     }
    ...
    if(nrow(A) > 0) {
      write.csv(A
                , file = paste(names(out_list)[i], ' frag_', j
                               , '_bovine_10bp_cluster_recom_class_union.csv', sep = '')
                , quote = F, row.names = F)
    }
  }
}
...
frag1 <- read_as_list(path = '~/Analysis/bovine/viral_v2/') %>%
  ...
  lin_frag1_identification(recom_matrix = lib_matrix, libs = 8
                           , gff_path = '~/Analysis/reference/bovine_edit_gff.csv'
                           , UTR5 = 210, UTR3 = 30741, length = 31032) %>%
  cut_by_n_bp(genome_length = 31032, bin = 10, frag = 1)
...
############################################################################################
## sgmRNA qname extraction
A <-  as.data.frame(matrix(data = '', nrow = 0, ncol = 0))
out_list <- list(A,A,A,A,A,A,A,A)
bed <- read_as_list(path = '~/Analysis/bovine/viral_v2/')
for(j in 2) {
  frag <- bed%>%
    specific_frag_format_organ(frag = j) %>% 
...
for(j in 2) {
  ...
  for(i in seq_along(frag)) {
    frag_extract <- frag[[i]]
    frag_extract <- frag_extract[frag_extract$class %in% 'DiRNA', ]
    f2_di <- frag_extract 
    print(nrow(f2_di))
    A <- out_list[[i]]
    A <- rbind(A, f2_di)
...
bed <- read_as_list(path = '~/Analysis/bovine/viral_v2/')
frag <- read_as_list(path = '~/Analysis/bovine/viral_v2/') %>%
  specific_frag_format_organ(frag = 1)  %>%
  lin_frag1_identification(recom_matrix = lib_matrix, libs = 8
                           , gff_path = '~/Analysis/reference/bovine_edit_gff.csv'
                           , UTR5 = 210, UTR3 = 30741, length = 31032)
